	...
	Column(
		...
		Column(
			...
			OtpView(
				...
				isValidOtp = true,
				onTextChanged = enterOtp
			)

			Spacer(modifier = Modifier.height(Dimens.dp_24))

			OtpNotReceivedText(name = uiState.farmerName)
		...
		if (isRecordSale) ContinueWithoutInsurance(continueWithoutInsurance)
	}
...
fun ContinueWithoutInsurance(
	...
	Text(
		text = stringResource(R.string.kyc_continue_without_insurance),
		style = TextStyle(
...
fun ResendOtp(
	...
	var timer: String? by remember { mutableStateOf(null) }
	...
	AnimatedVisibility(visible = timer.isNullOrBlank().not()) {
	...
	Row {
		ResendOtpButton(R.string.by_sms, timer.isNullOrBlank()) {
			startTimer = false
			sendOtpViaSms()
			startTimer = true
		}
		...
		ResendOtpButton(R.string.by_call, timer.isNullOrBlank()) {
			startTimer = false
			sendOtpViaCall()
			startTimer = true
		}
